...
[CustomPropertyDrawer(typeof(VectorAttribute))]
    public class VectorAttributeDrawer : PropertyDrawer
    {
		...
		{
			VectorAttribute vector = (VectorAttribute)attribute;
		...
		protected T[] DrawFields<T>(Rect rect, T[] vector, string mainLabel, System.Func<Rect, GUIContent, T, T> fieldDrawer, VectorAttribute vectors, GUIContent originalGuiContent)
		{
			T[] result = vector;
